...
class ReservationService(
    ...
    /**
     * 1. 락을 획득한다.
     * 2. token 을 통해 queue 를 검증한다. (Processing 상태인지 확인한다.)
     * 3. queue 에서 user 를 추출한다.
     * 4. user, concert, schedule, seat 들의 존재 여부를 검증한다.
     * 5. reservation 을 생성한다.
     * 6. 예약이 완료되면 좌석의 상태를 UNAVAILABLE 로 변경한다.
     * 7. 락을 반환한다.
     */
    fun createReservations(
        ...
        existUser(reservationRequest.userId)
        ...
        return createReservationsWithLock(
            token = token,
    ...
    fun createReservationsWithLock(
        token: String,
        reservationRequest: ReservationServiceDto.Request,
    ): List<ReservationServiceDto.Result> =
        reservationManager
