...
class QueueRedisRepository(
    ...
    fun addToWaitingQueue(
        ...
        redisTemplate.opsForZSet().add(WAITING_QUEUE_KEY, "$token:$userId", score)
    }
    ...
    fun updateToProcessingQueue(
        ...
        redisTemplate.opsForSet().add(PROCESSING_QUEUE_KEY, "$token:$userId:$expirationTime")
    }
    ...
    fun isProcessingQueue(token: String): Boolean {
        val pattern = "$token:*"
        return redisTemplate.opsForSet().members(PROCESSING_QUEUE_KEY)?.any { it.startsWith(pattern) } ?: false
    }
    ...
    fun getWaitingQueuePosition(
        token: String,
        userId: String,
    ): Long = redisTemplate.opsForZSet().rank(WAITING_QUEUE_KEY, "$token:$userId") ?: 0
     ...
     */
    fun getProcessingQueueCount(): Long = redisTemplate.opsForSet().size(PROCESSING_QUEUE_KEY) ?: 0
    ...
    fun removeExpiredWaitingQueue(currentTime: Long) {
        redisTemplate.opsForSet().members(WAITING_QUEUE_KEY)?.forEach {
            val (token, userId, expirationTime) = it.split(":")
            if (expirationTime.toLong() < currentTime) {
                redisTemplate.opsForSet().remove(WAITING_QUEUE_KEY, it)
            }
        }
    }

    /**
     * 취소되거나 완료되지 않은 상태의 PROCESSING 대기열을 삭제한다.
     */
